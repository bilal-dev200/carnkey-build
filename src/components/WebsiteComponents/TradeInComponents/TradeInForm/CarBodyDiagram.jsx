...
export const CarBodyDiagram = ({ selectedSidebar, damageDetails, setDamageDetails, selectedPart, setSelectedPart, selectedDamageType, setSelectedDamageType, selectedSubcategory, setSelectedSubcategory }) => {
  ...
  const handlePartClick = (event) => {
    ...
    if (selectedSidebar === 'Body' && selectableBodyParts.includes(clickedValue)) {
        ...
        return {
          ...
          [selectedPart]: {
            ...
            damageTypes: alreadySelected
              ? existing.filter((d) => d.name !== type.name) // remove if already selected
              : [...existing, type], // add if not selected
            subcategory: null, // reset subcategory for new damage
          },
        };
      });
    }
  ...
  useEffect(() => {
    ...
    setSelectedDamageType(null);
    setSelectedSubcategory(null);
  }, [selectedSidebar, setDamageDetails, setSelectedDamageType, setSelectedSubcategory, setSelectedPart]);
        ...
        <div className="flex flex-col lg:flex-row items-start justify-center p-4 w-full">
          ...
          <div className="w-full lg:min-w-1/2 flex flex-col items-center lg:items-start lg:pl-8">
            {selectedPart && damageOptions[selectedPart] && (
              <div className="p-6 bg-white rounded-lg shadow-md w-full md:max-w-lg mb-8">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {damageOptions[selectedPart].types.map((type) => {
                    const isSelected =
                      damageDetails[selectedPart]?.damageTypes?.some((t) => t.name === type.name);
