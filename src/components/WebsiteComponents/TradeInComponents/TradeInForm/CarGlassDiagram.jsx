...
export const CarGlassDiagram = ({ selectedSidebar, damageDetails, setDamageDetails, selectedPart, setSelectedPart, selectedDamageType, setSelectedDamageType, selectedSubcategory, setSelectedSubcategory }) => {
  // Define the list of parts that are selectable when 'Body' sidebar is active
  // These values correspond to the 'data-svg-clicked-val' attributes in the SVG
  const selectableBodyParts = [
    ...
    "Right Mirror",
    "roof",
    "wheel", // Including wheels as they are present in the SVG and might be selectable in other contexts
    "left front glass",
    "left rear glass",
    "windshield",
    "rear window",
  ...
  // Dummy data for damage types, subcategories, and costs
  // This data is structured to match the image provided.
  const damageOptions = {
    "Hood": {
      types: [
      ...
      description: "Dents can be repaired if they are not cracked, have sharp indents, or are very large. If the body panel is made of aluminum, a dent cannot be repaired.",
      subcategories: [
        { name: "Repair", cost: 0 },
    ...
    "windshield": {
      ...
      description: "Windshield damage details.",
      subcategories: [
        { name: "Repair", cost: 50 },
    ...
    "right front glass": {
      ...
      description: "Right front glass damage details.",
      subcategories: [
    ...
    "right rear glass": {
      ...
      description: "Right rear glass damage details.",
      subcategories: [
    ...
    "rear window": {
      ...
      description: "Rear window damage details.",
      subcategories: [
        { name: "Repair", cost: 60 },
    ...
    "left rear glass": {
      ...
      description: "Left rear glass damage details.",
      subcategories: [
        { name: "Replace", cost: 250 },
      ],
    },
    "left front glass": {
      ...
      description: "Left front glass damage details.",
      subcategories: [
        { name: "Replace", cost: 300 },
      ],
    },
    ...
    "Front Left Light": {
      types: [{ name: "Cracked", cost: 120 }, { name: "Non-functional", cost: 250 }],
    ...
    "Rear Left Light": {
      types: [{ name: "Cracked", cost: 100 }, { name: "Non-functional", cost: 200 }],
    ...
    "Front Left Fender": {
      types: [{ name: "Dent", cost: 180 }, { name: "Scratch", cost: 90 }],
      description: "Front left fender damage.",
      subcategories: [{ name: "Repair", cost: 0 }, { name: "Replace", cost: 400 }],
    },
    "Rear Left Fender": {
      types: [{ name: "Dent", cost: 180 }, { name: "Scratch", cost: 90 }],
    ...
    "Front Left Doors": {
      types: [{ name: "Dent", cost: 250 }, { name: "Scratch", cost: 120 }, { name: "Ding", cost: 80 }],
      description: "Front left door issues.",
      subcategories: [{ name: "Repair", cost: 0 }, { name: "Replace", cost: 600 }],
    },
    "Back Left Door": {
      types: [{ name: "Dent", cost: 250 }, { name: "Scratch", cost: 120 }, { name: "Ding", cost: 80 }],
    ...
    "Left Rocker": {
      types: [{ name: "Scrape", cost: 100 }, { name: "Rust", cost: 200 }],
      description: "Left rocker panel damage.",
      subcategories: [{ name: "Repair", cost: 0 }, { name: "Replace", cost: 300 }],
    },
    "Right Rocker": {
      types: [{ name: "Scrape", cost: 100 }, { name: "Rust", cost: 200 }],
    ...
    "Left Mirror": {
      types: [{ name: "Cracked Glass", cost: 50 }, { name: "Broken Casing", cost: 150 }],
    ...
    "roof": {
      types: [{ name: "Hail Damage", cost: 400 }, { name: "Large Dent", cost: 350 }],
  ...
  const handlePartClick = (event) => {
    ...
    console.log('clickeed', clickedValue)
    if (selectedSidebar === 'Glass' && selectableBodyParts.includes(clickedValue)) {
      // Toggle selection of the part
      setSelectedPart(prevSelectedPart => {
        if (prevSelectedPart === clickedValue) {
          // If the same part is clicked again, deselect it and remove its damage details
          ...
          // If a new part is selected, clear previous damage type/subcategory selections
          setSelectedDamageType(null);
          setSelectedSubcategory(null);
          return clickedValue;
        }
      });
    } else if (selectedSidebar !== 'Glass') {
      // If the sidebar is not 'Body', clear any active selection
  ...
  // Handler for selecting a damage type
  // const handleDamageTypeSelect = (type) => {
  //   setSelectedDamageType(type);
  //   setSelectedSubcategory(null); // Reset subcategory when damage type changes
  //   if (selectedPart) {
  ...
  //         damageType: type,
  //         subcategory: null, // Clear subcategory for this part
  //       }
  //     }));
  //   }
  // };
  ...
  const handleDamageTypeSelect = (type) => {
    ...
    setDamageDetails((prevDetails) => {
      ...
      const alreadySelected = existing.find((d) => d.name === type.name);

      return {
        ...prevDetails,
        [selectedPart]: {
          ...prevDetails[selectedPart],
          // toggle type (add if not in, remove if in)
          damageTypes: alreadySelected
            ? existing.filter((d) => d.name !== type.name)
            : [...existing, type],
          subcategory: null, // keep subcategory reset like before
        },
      };
    });
  };
  ...
  return (
    <>
      {/* Main container for the Car Body Diagram and Damage Selection UI */}
      <div className="flex flex-col items-start justify-center p-4 w-full">
        ...
        <div className="flex flex-col lg:flex-row items-start justify-center p-4 w-full">
          {/* Left Section: Vehicle Diagram SVG */}
          <div className="w-full lg:min-w-1/2 flex justify-center lg:justify-end mb-8 lg:mb-0 lg:pr-8">
            ...
            >
              ...
              <polygon
                type="Front Glass"
                side="Left"
                data-svg-clicked-val="left front glass"
                points="75.3,81.7 75.3,120.3 92.5,120.3 92.5,95.9 92.5,95.9"
                className={`ExtSVGElem ${selectedPart === 'left front glass' ? 'selected-part' : ''}`}
              ...
              <polygon
                type="Back Glass"
                side="Left"
                data-svg-clicked-val="left rear glass"
                points="75.3,161.3 75.3,122.7 92.5,122.7 92.5,147.1 92.5,147.1"
                className={`ExtSVGElem ${selectedPart === 'left rear glass' ? 'selected-part' : ''}`}
                onClick={handlePartClick}
              />
              {/* Windshield */}
              <path
                type="Windshield"
                side=""
                data-svg-clicked-val="windshield"
                ...
                className={`ExtSVGElem ${selectedPart === 'windshield' ? 'selected-part' : ''}`}
                onClick={handlePartClick}
              />
              {/* Rear Window */}
              <path
                type="Rear Window"
                side=""
                data-svg-clicked-val="rear window"
                ...
                className={`ExtSVGElem ${selectedPart === 'rear window' ? 'selected-part' : ''}`}
                onClick={handlePartClick}
              />
              {/* Roof */}
              <rect
                ...
                type="Roof"
                side=""
                data-svg-clicked-val="roof"
                x="94.9"
                y="96.8"
                width="48.3"
                height="48.3"
                className={`ExtSVGElem enabled ${selectedPart === 'roof' ? 'selected-part' : ''}`}
              // onClick={handlePartClick}
              />
              {/* Hood */}
              <g>
              ...
              <g>
                ...
                <g>
                  ...
                  <path
                    ...
                    type="Front Door"
                    side="left"
                    data-svg-clicked-val="Front Left Doors"
                    ...
                    className={`ExtSVGElem enabled ${selectedPart === 'Front Left Doors' ? 'selected-part' : ''}`}
                  // onClick={handlePartClick}
                  />
                </g>
                {/* Left Fender */}
              ...
              <g>
                ...
                <g>
                  <path
                    ...
                    type="Back Door"
                    side="right"
                    data-svg-clicked-val="Back Right Door"
                    ...
                    className={`ExtSVGElem enabled ${selectedPart === 'Back Right Door' ? 'selected-part' : ''}`}
                  // onClick={handlePartClick}
                  />
                </g>
                <g>
                  ...
                  <path
                    ...
                    type="Front Door"
                    side="right"
                    data-svg-clicked-val="Front Right Door"
                    ...
                    className={`ExtSVGElem enabled ${selectedPart === 'Front Right Door' ? 'selected-part' : ''}`}
                  // onClick={handlePartClick}
                  />
                </g>
                {/* Right Fender */}
...
export default CarGlassDiagram;